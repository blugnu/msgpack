...
// BeginArray writes the msgpack type and length of an array to the
...
// BeginMap writes the msgpack type and length of a map to the
// current writer using the most efficient msgpack encoding possible
...
// BeginString writes the msgpack type and length of a string to the
// current writer using the most efficient msgpack encoding possible
...
func (enc Encoder) BeginString(len int) error {
	switch {
	case len < 32:
...
func (enc Encoder) EncodeString(s string) error {
	if err := enc.BeginString(len(s)); err == nil {
